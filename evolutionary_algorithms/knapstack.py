...
class Algorithm():
    ...
    def calculate_fitness(self, genes):
        ...
        for index, gene in enumerate(genes):
            if gene == 1:
                utility += self.data[index][1]
                weight += self.data[index][0]
        return weight if weight <= self.capacity else 0
...
def get_input():
    ...
    while(1):
        w = raw_input('Please enter weight of object: ')
        if w == '0':
            break
        p = raw_input('Utility: ')
        objects.append((int(w), float(p)))
    return objects
